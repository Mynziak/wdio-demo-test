...
export const config: WebdriverIO.Config = {
  ...
  // - @wdio/applitools-service, @wdio/browserstack-service, @wdio/sauce-service
  ...
  // baseUrl: "https://www.misterspex.de/",
  ...
  connectionRetryCount: 2, //3 by default
  ...
  reporters: [
    // ['dot', { outputDir: './reports/dot' }],
    ['spec', { outputDir: './reports/spec' }],
    ['allure', {
  ...
  mochaOpts: {
     ...
     * tests because they can take a bit longer.
     */
    timeout: 3 * 60 * 1000, // - !!! Timeout for test session duration!!!
  },
  ...
  afterTest: async function (
    ...
    if (error) {
      if (error) {
        const screenshot = await browser.takeScreenshot(); //take screenshot on failure  
      }
    }
  },
};
